# Samuel Phase 0 ù Render deploy
...
| **sam-agent** | LiveKit Agents worker (private) | none ù registers with LiveKit Cloud |
...
3. **Apply** ù creates `sam-token`, `sam-voice-portal`, `sam-agent`
...
```
V2V turn ù: eou=ùms + ttft=ùms + ttfb=ùms = ùms
```
...
2. Archive `ThePokerNinja/charles` `voice_pwa/` ù do not dual-run voice products
...
| Agent hears itself (console) | Expected without WebRTC AEC ù test in browser with mic |
