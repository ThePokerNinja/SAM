...
class RoutedSamuelAgent(Agent):
    ...
    async def llm_node(self, chat_ctx, tools, model_settings):
        ...
        if appointment_pack:
            ...
            # Phone/browser intake: keep proposal tools available; drop page-only writers
            # that the deterministic builder path handles (research + question list).
            selected = [
                tool
                for tool in available
                if tool_callable_name(tool) in VOICE_INTAKE_LLM_TOOLS
            ]
        else:
            selected = (
                available
